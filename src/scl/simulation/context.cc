...
using GlobalCtx = scl::details::GlobalContext;

namespace {

std::vector<scl::Bitmap> initBitmaps(std::size_t number_of_parties) {
  std::vector<scl::Bitmap> bms;
...
GlobalCtx GlobalCtx::create(std::size_t number_of_parties,
  ...
  std::vector<Time::TimePoint> clocks(number_of_parties);
  std::vector<Bitmap> recv_map = initBitmaps(number_of_parties);
  ...
  return {number_of_parties,
          ...
          clocks,
          recv_map,
          Bitmap(number_of_parties),
          std::move(hooks)};
}

scl::Time::Duration GlobalCtx::LocalContext::lastEventTimestamp() const {
...
scl::Time::Duration GlobalCtx::LocalContext::elapsedTime() const {
  const Time::Duration most_recent = lastEventTimestamp();
  return most_recent + (Time::now() - m_gctx.clocks[m_id]);
}

void GlobalCtx::LocalContext::startClock() {
  m_gctx.clocks[m_id] = Time::now();
}
...
long double rttSeconds(const scl::ChannelConfig& config) noexcept {
...
long double throughputNoLoss(const scl::ChannelConfig& config) noexcept {
...
long double throughputLoss(const scl::ChannelConfig& config) noexcept {
...
scl::Time::Duration recvTimeTCP(const scl::ChannelConfig& config,
                                std::size_t n) {
  ...
  return std::chrono::duration_cast<scl::Time::Duration>(t_sec);
}

// Computes the delay that sending an amount of bytes would incur.
scl::Time::Duration adjustSendTime(const scl::ChannelConfig& config,
                                   scl::Time::Duration send_time,
                                   std::size_t n) {
  if (config.type() == scl::ChannelConfig::NetworkType::TCP) {
...
scl::Time::Duration GlobalCtx::LocalContext::currentTimeOf(
...
std::ostream& scl::details::operator<<(
    std::ostream& os,
    const details::GlobalContext& global_ctx) {
  os << "GLOBAL_CTX{";
